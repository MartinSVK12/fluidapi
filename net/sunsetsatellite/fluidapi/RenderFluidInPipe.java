...
public class RenderFluidInPipe extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileEntity1, double d2, double d4, double d6, float f8) {
        ...
        World blockAccess = ModLoader.getMinecraftInstance().theWorld;
        Block block = mod_FluidAPI.fluidPipe;
        ...
        float mapped = (float) mod_FluidAPI.map(amount,0.0d,1.0d,0.0d,0.5d);
        ...
        if(fluidId != -1)
        drawBlock(this.getFontRenderer(), ModLoader.getMinecraftInstance().renderEngine, fluidId, 0,0, 0, 0);
